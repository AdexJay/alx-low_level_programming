...
(((n << 16) >> 24) << 16) | (n >> 24))
/**
 * verify - verify the file to check if it is ELF
 * @e_ident: the ELF struct
 * return: no return, is a void funciton
 */
void verify(unsigned char *e_ident)
{
	...
	{
		dprintf(STDERR_FILENO, "Error: thisfile not is a valid ELF\n");
		exit(98);
	}
}
...
/**
 * version - prints the file version
 * @e_ident: the ELF struct
 * return: no return
 */
 ...
 * @argv: the args
 * section header: the header of the function is holberton.h
 * return: 0 in success
 */
